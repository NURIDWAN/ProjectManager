...
export default function Edit({ bast, availableBaps }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        bap_id: String(bast.bap_id),
        tanggal: bast.tanggal?.split('T')[0] ?? '',
        work_items: (bast.work_items ?? []).map((item) => ({
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        put(`/basts/${bast.id}`, {
            ...
            onError: () => {
                toast.error('Gagal memperbarui BAST. Periksa form dan coba lagi.');
            },
        });
    };
