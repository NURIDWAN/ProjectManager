...
export default function Create({ clients, workReports, selectedClientId }: Props) {
    const [clientFilter, setClientFilter] = useState(selectedClientId || '');

    const { data, setData, post, processing, errors } = useForm({
        client_id: selectedClientId || '',
        tanggal: new Date().toISOString().split('T')[0],
        work_report_ids: [] as number[],
    });
    ...
    const handleClientFilterChange = (value: string) => {
        ...
        // Reload page with client filter for server-side filtering
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post('/baps', {
            onSuccess: () => {
                toast.success('BAP berhasil dibuat.');
            },
            onError: () => {
                toast.error('Gagal membuat BAP. Periksa form dan coba lagi.');
            },
        });
    };
